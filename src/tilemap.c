...
TILEMAP_TILEMAP* tilemap_create( bstring ps_path_in ) {
   TILEMAP_TILEMAP* ps_tilemap_out = malloc( sizeof( TILEMAP_TILEMAP ) );
   ezxml_t ps_xml_tilemap = NULL, ps_xml_layer0 = NULL, ps_xml_tile = NULL,
      ps_xml_props = NULL, ps_xml_prop_iter = NULL, ps_xml_tiledata = NULL;
   bstring ps_tiledata_path = NULL, ps_tiledata_filename = NULL;

   /* Verify memory allocation. */
   if( NULL == ps_tilemap_out ) {
      DBG_ERR( "Unable to allocate tile map." );
   } else {
      memset( ps_tilemap_out, 0, sizeof( TILEMAP_TILEMAP ) );
   }

   /* Verify the XML file exists and open or abort accordingly. */
   if( !file_exists( ps_path_in ) ) {
      DBG_ERR_FILE( "Unable to load tile map", ps_path_in->data );
      return NULL;
   }
   ps_xml_tilemap = ezxml_parse_file( ps_path_in->data );

   /* Load the tileset data if it exists, or just let cleanup run if it       *
    * doesn't.                                                                */
   ps_xml_tiledata = ezxml_child( ps_xml_tilemap, "tileset" );
   ...
   if( file_exists( ps_tiledata_path ) ) {
      /* Load the tile data. */
      ps_tilemap_out->tileset = graphics_create_tileset( ps_tiledata_path );
   ...
   ezxml_free( ps_xml_tilemap );

   return ps_tilemap_out;
}
